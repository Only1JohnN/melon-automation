...
- Name: ui/partners/settings.spec.ts >> @partners @settings @smoke >> Business >> Details >> should upload a business logo successfully
- Location: tests/ui/partners/settings.spec.ts:332:11
...
Call log:
  - waiting for locator('label').filter({ hasText: /upload photo/i }).nth(1)
