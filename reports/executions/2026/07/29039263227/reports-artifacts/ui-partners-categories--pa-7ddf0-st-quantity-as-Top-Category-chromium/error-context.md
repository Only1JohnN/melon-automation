...
- Name: ui/partners/categories.spec.ts >> @partners @categories @smoke >> should display category with highest quantity as Top Category
- Location: tests/ui/partners/categories.spec.ts:878:7
...
Call log:
  - waiting for locator('div').filter({ hasText: /^Top Category/ }).first().locator('span').last()
