...
export default async function ExecutionPage({
  ...
  const [overview, applications, groupedTests, groupedFailures] =
    await Promise.all([
      getExecutionOverview(id),
      getExecutionApplications(id),
      getGroupedExecutionTests(id),
      getGroupedExecutionFailures(id),
    ]);
      ...
      <div className="space-y-6 p-6">
        ...
        <div className="rounded-3xl border border-slate-800 bg-[#111827] p-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold">Execution Summary</h2>
              <p className="mt-2 text-slate-400">
                {new Date(overview.executedAt).toLocaleString()}
              </p>
            </div>
          ...
          <div className="mt-8 grid grid-cols-2 gap-6 md:grid-cols-4">
            ...
            <div>
              ...
              <p className="mt-1 text-3xl font-bold">{overview.passRate}%</p>
            </div>
          </div>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {applications.map((app) => (
                <tr key={app.name} className="border-t border-slate-800">
